...
# crop-режим: ВЫРЕЗАННЫЙ рисунок (img_s3 от paddle/mineru) — описать детально +
# извлечь ДОСЛОВНО формулы/подписи/числа ВНУТРИ. На кропе (полное разрешение)
# мелкий текст читаем, в отличие от уменьшенной страницы (figure-sweep его теряет).
_CROP_SYSTEM = """\
Ты — специалист по технической документации. Тебе дают ВЫРЕЗАННЫЙ рисунок, схему,
график, диаграмму или таблицу-картинку из техдокумента (любая отрасль). Опиши его
по делу и извлеки весь текст внутри."""

_CROP_PROMPT = (
    "Опиши ПО-РУССКИ этот рисунок: что изображено, ключевые элементы, связи, "
    "инженерный/смысловой смысл. ОБЯЗАТЕЛЬНО извлеки ДОСЛОВНО весь текст, формулы, "
    "подписи, числа и обозначения ВНУТРИ рисунка (в ячейках/блоках, на осях, стрелках, "
    "в легенде) — это нужно для поиска. Формулы и выражения записывай как есть. Без воды."
)
...
class VisionClient:
    ...
    async def describe_crop(self, image_bytes: bytes) -> str:
        """Вырезанный рисунок (img_s3) → детальное описание + ДОСЛОВНЫЙ текст/формулы
        ВНУТРИ. На кропе (полное разрешение) мелкие формулы/подписи читаемы, в отличие
        от уменьшенной страницы (describe_figure их теряет)."""
        return await self._complete(_CROP_SYSTEM, _CROP_PROMPT, image_bytes)
