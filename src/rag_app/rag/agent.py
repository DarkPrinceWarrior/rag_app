...
- "doc_plus_memory": нужны документы И контекст прошлых обсуждений/предпочтений,
  ИЛИ продолжение твоего же прошлого ответа. Примеры: «а что там с поставкой,
  которую обсуждали», «продолжи по тому договору», «распиши пункт 5 подробнее»,
  «раскрой предыдущий ответ» (это про разделы ТВОЕГО прошлого ответа, по истории
  диалога, а НЕ новый поиск рисунка/раздела).
...
- "agentic_multi_step": нужно несколько поисков / сбор по всему документу /
  ...
  СЮДА ЖЕ — запрос про конкретный пункт/раздел/главу документа по НОМЕРУ:
  «про что пункт 5», «что в разделе 3», «перескажи главу 4», «распиши раздел 5».
  Такой раздел берётся инструментом get_chapter по номеру заголовка; обычным
  поиском номерной раздел не находится (всплывают случайные фрагменты/рисунки).
- "out_of_scope": запрос НЕ про содержание документов библиотеки. Сюда входят:
  перевод присланного/выделенного текста или просьба перевести документ;
  действие над присланным текстом (сократи, переформулируй, сделай саммари);
  приветствие, болтовня, вопрос о возможностях; просто вставленный текст без
  вопроса к документам. Примеры: «переведи этот текст», «переведи документ на
  английский», «переведи на английском», «привет», «что ты умеешь»,
  «сделай саммари вот этого: …».
...
_MULTI_HINTS = (
    ...
    # структурные запросы по номеру раздела/пункта/главы → get_chapter
    "что в раздел", "про что пункт", "про что раздел", "перескажи глав",
    "перескажи раздел", "распиши раздел", "распиши пункт", "что в пункт", "что за пункт",
)
_MEMORY_HINTS = (
...
- get_chapter(query): раздел/глава/пункт ЦЕЛИКОМ по номеру из заголовка (query =
  номер, напр. "3", "4.2", "5"). ОБЯЗАТЕЛЬНО используй его, когда спрашивают про
  пункт/раздел/главу по номеру: «про что пункт 5», «что в разделе 4.2», «перескажи
  главу 3», «распиши раздел 5» — он собирает ВСЕ фрагменты этого раздела по
  заголовку (обычный search_chunks номерной раздел НЕ находит — всплывают рисунки);
...
- find_figure(query): найти рисунок/таблицу по ЯВНО названному номеру из вопроса
  ...
  ссылается на номер рисунка/figure/таблицы («что на рис. 9.1», «опиши figure 3»,
  «таблица 2»): кроп рисунка попадёт в контекст для визуального разбора, даже если
  обычный поиск его не поднял;
