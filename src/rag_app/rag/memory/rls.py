...
сессионные GUC текут между клиентами (§3.7). Ошибка установки контекста
прерывает транзакцию: память защищена FORCE RLS и работает fail-closed.
"""
...
async def apply_scope_guc(session: AsyncSession, scope: MemoryScope) -> None:
    """Выставить scope-контекст RLS; ошибка должна прервать транзакцию."""
