...
| Резервная копия БД | возраст последней успешной full/diff pgBackRest | ≤36 ч | непрерывно |
| Архивирование WAL | возраст последнего успешного `pgbackrest check` | ≤2 ч | непрерывно |
| Объектное хранилище | возраст последнего успешного versioned MinIO mirror | ≤2 ч | непрерывно |
| Конфигурация SSO | возраст последнего offline-export realm | ≤8 суток | непрерывно |
...
Backup-метрики читают только content-free epoch-файлы из
`/var/lib/docragenslate/backup-metrics`; пути, имена объектов, идентификаторы и
секреты в Prometheus не передаются. Нулевой timestamp означает, что операция ещё
не завершалась успешно, и штатно активирует соответствующий alert после
включения backup units.
