...
### 15.9 Живая верификация (2026-06-16)

- **Реальный чат e2e** (`scripts/_e2e_memory_chat.py`, in-process ASGI против `POST /api/chat`): двусессионный тест — факт сообщён в сессии 1, всплыл в сессии 2 (новый тред) через настоящую цепочку `chat.py`. **ПАМЯТЬ РАБОТАЕТ КРОСС-СЕССИОННО.** Тест поймал и зафиксировал **2 реальных бага**, которые статика/юниты пропустили: (1) `UnboundLocalError` — `user` локализовалась в `gen()` из-за позднего переприсваивания; (2) недоделанный маршрут `memory_only` тянул документы и давал юлящий ответ → теперь пропускает документный поиск и отвечает из памяти (`MEMORY_ONLY_SYSTEM_PROMPT`).
- **Playwright (браузер, временный auth-off инстанс):** страница «Память» — добавление записи через форму → появляется в списке; чат — вопрос про добавленную запись → трасса «🧠 учтено из памяти: 1» и ответ из памяти; сайдбар истории + восстановление чата по `?sid=` после ухода в меню (фикс «чаты пропадают» подтверждён визуально); тумблер «Временный».
- **Сравнение с Mem0** (`scripts/bench_mem0.py`, Mem0 2.0.6 на нашем Qwen3.5+эмбеддере, тот же selftest и LLM-судья): из коробки Mem0 **НЕ работает** на on-prem — дефолтный `ADDITIVE_EXTRACTION_PROMPT` ~8.4к токенов один превышает наш ctx 8192 (плюс правки: embedder `dimensions`, search `filters=`, `enable_thinking`). С коротким extraction-промптом Mem0=4/4. Наш `internal` после фикса scope-downgrade (project/document-факты без контекста опускались в сироты → не доставались) тоже 4/4. **Вывод: на мини-наборе ничья по точности**; токены/латентность сопоставимы (internal 167т / mem0 136т; ретрив ~17 мс у обоих). Наши преимущества — scope-изоляция (project/document/thread, §1), гибрид dense+BM25, gate, RLS, 152-ФЗ; Mem0-OSS-default — user-global, dense-only (chroma). Строгий вердикт — за LoCoMo/LongMemEval.

Коммиты: `ff6c4c7` (слой памяти + e2e-фиксы), `0ed6d48` (scope-downgrade + Mem0-бенч); прод выровнен, smoke зелёный.

**Открытые доводки (P2):** калибровка порогов извлечения/gate и LLM-судья на реальных LoCoMo/LongMemEval; FORCE-RLS + `BYPASSRLS`-роль воркера после нагрузочной верификации GUC; bench против поднятого Mem0 OSS с гибридным стором (pgvector) для честного по ретриву сравнения.
